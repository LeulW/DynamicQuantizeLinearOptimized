...
void openmp_1(vector<float> const &x, vector<uint8_t> &y, unsigned long long const& numElts){

    unsigned long long i;
    float min_per_thread[NUM_THREADS*PADDING] = {x[0]};
    float max_per_thread[NUM_THREADS*PADDING] = {x[0]};
    unsigned long long workPerThread = (numElts-1)/NUM_THREADS + 1;
    #pragma omp parallel private (i) num_threads(NUM_THREADS)
    {
        int id = omp_get_thread_num();
        for (i=id*workPerThread; i<(id+1)*workPerThread && i<numElts; i++){
            min_per_thread[id*PADDING] = min(min_per_thread[id*PADDING], x[i]);
            max_per_thread[id*PADDING] = max(max_per_thread[id*PADDING], x[i]);
        }
    }

    float min_x, max_x = x[0];
    for (i=0; i<NUM_THREADS; i++){
        min_x = min(min_per_thread[i*PADDING], min_x);
        max_x = max(max_per_thread[i*PADDING], max_x);
    } 

    float const y_scale = (max(0.0f, max_x) - min(0.0f, min_x))/255;
    // cout<<unsigned(y_scale)<<endl;
...
int main(int argc, char *argv[]){
    ...
    cout<<"Optimization 4 time: "<<total_time<<endl;
    if (test < 3){
        printResult(y);
    }
